...
export function ActorsSection() {
  const actors = [
    ...
    },
    {
      ...
      icon: <Store className="h-7 w-7 text-[var(--expo-navy)]" />,
  ...
  return (
    <section id="beneficios" className="bg-[var(--expo-bg)] py-16 md:py-24 border-t-2 border-[var(--expo-line)] relative">
        ...
        <div className="mt-14 grid grid-cols-1 gap-8 lg:grid-cols-3">
          {actors.map((actor, idx) => (
            <motion.div
              ...
              className="flex flex-col justify-between overflow-hidden rounded-2xl border-3 border-[var(--expo-navy)] bg-white shadow-[7px_7px_0_var(--expo-navy)] transition-all hover:-translate-y-1"
            >
              {/* Card Header Banner */}
              <div>
                ...
                <div className="p-6 space-y-3">
                  {actor.features.map((feat, fIdx) => (
                    <div key={fIdx} className="flex items-start gap-2.5">
                      <CheckCircle2 className="h-4 w-4 text-[var(--expo-green)] shrink-0 mt-0.5" />
              ...
              <div className="border-t border-slate-100 bg-slate-50 px-6 py-3.5 flex items-center justify-between text-xs font-mono font-bold text-slate-500">
                ...
                <span className="flex items-center gap-1 text-[var(--expo-blue)]">
                  Beneficios <ArrowUpRight className="h-3.5 w-3.5" />
                </span>
              </div>
            </motion.div>
          ))}
